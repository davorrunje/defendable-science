...
| [0041](0041-dashboard-generated-by-the-cli.md) | `dashboard.md` is generated by `defendable-science progress dashboard`, the file's only writer (the skill calls it rather than projecting); a table per level plus a conditional detail line, no timestamp in the artifact, gaps named never counted, an unreadable artifact kept as a visibly `unknown` row with exit 1 | accepted |
| [0042](0042-depth-mode-matrix-cells-provenance.md) | A depth-mode digest feeds the concept matrix through `write_depth_cells`, the same cells block extraction uses; provenance is the *absence* of `status.extraction`, not a new field, and `extract render`'s default batch widens to any artifact with recorded cells | accepted |
...
| [0044](0044-devcontainer-for-maintainer-development.md) | Single-flavor devcontainer for maintainer development, adapted from `mononet` — `~/.claude` persists across rebuilds via a named volume, Claude Code sessions are shared between host and container via a host-side symlink scoped to the exact host path opened | accepted |
