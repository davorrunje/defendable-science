...
- **Plugin:** the `literature` skill; the citation-graph engine (OpenAlex + S2
  clients, snowballing, intent classification, clustering); the CSL-JSON
  registry loader/patcher (§4) and the `triage.yml` sidecar reader; the
  acquisition ladder, match gate, quarantine, and fixity/mirror wrappers (§5);
  reference digests. The PRISMA-style log and concept-centric matrix (§3) are
  **not** generated by a CLI command — `position` mode produces them by
  following the described procedure over the triage sidecar's fields by
  hand; a generator is a filed follow-up, not shipped. Deps: HTTP client +
