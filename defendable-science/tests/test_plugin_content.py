...
def test_no_skill_hard_codes_the_research_tree(skill: Path) -> None:
    ...
    for n, line in enumerate(lines, 1):
        ...
        if "docs/research/" in line:
            offenders.append(f"{skill.relative_to(_REPO_ROOT)}:{n}: {line.strip()}")
...
def test_guard_allows_paths_in_fenced_blocks() -> None:
    ...
    test_skill = Path(__file__).parent / "temp_test_skill.md"
...
"""
    ...
    try:
        ...
        for n, line in enumerate(lines, 1):
            ...
            if "docs/research/" in line:
                prose_offenders.append((n, line.strip()))
