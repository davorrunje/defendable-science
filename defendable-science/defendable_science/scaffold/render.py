...
#: The marker the ungenerated stub carries, and the one fact
#: :func:`is_ungenerated_dashboard` keys off. Written once, here, because the
#: writer and the reader of a marker must not be able to drift apart.
_UNGENERATED_MARKER = "Not yet generated"


def render_dashboard() -> str:
    """Render the dashboard stub ``init`` writes.

    Still a stub, and deliberately: ``init`` scaffolds from pure renderers and
    does not walk the tree, so it cannot project a real dashboard — and writing
    a plausible-looking one would be the dishonest option. What it can do is
    say so and name the command that *does* generate it
    (``defendable-science progress dashboard``, #130), so the reader has a next
    action rather than a claim they cannot reproduce.

    :returns: The stub's contents, carrying :data:`_UNGENERATED_MARKER` so
        ``check`` can tell a repo that has never been projected from a stale
        projection.
    """
    return (
        "<!-- GENERATED by `defendable-science progress dashboard` — never "
        "hand-edited. -->\n\n"
        "# Research dashboard\n\n"
        f"{_UNGENERATED_MARKER}. This file is a pure projection of the status\n"
        "frontmatter in each hypothesis / paper / thesis artifact; run\n"
        "`defendable-science progress dashboard` to generate it. Nothing here is\n"
        "ground truth — if this file and the frontmatter disagree, the\n"
        "frontmatter wins.\n"
    )


def is_ungenerated_dashboard(text: str) -> bool:
    """Return whether `text` is the stub :func:`render_dashboard` writes.

    :param text: A dashboard's contents.
    :returns: ``True`` when it is the never-yet-generated stub. ``check`` uses
        this to hold its tongue about a repo that has no artifacts *and* has
        never been projected, which is a complete state rather than a stale one.
    """
    return _UNGENERATED_MARKER in text
